...
import { addMessageToHistorial } from "../../store/slices/chat/chatSlice";
...
const ChatMessage = ({ socket }) => {
	...
	useEffect(() => {
		socket.on("send-message", (payload) => {
			console.log(payload);
			dispatch(addMessageToHistorial(payload));
		});
		...
		<div className="flex flex-col mb-20 w-2/4 mx-2 my-10">
			<div className="flex sm:items-center border-b-2 border-gray-200">
				...
				<div className="relative">
					...
					<img src={recipientUser?.urlImg} alt="" className="w-10 sm:w-16 h-10 sm:h-16 rounded-full mx-3" />
				</div>
			...
			<div id="messages" className="flex flex-col space-y-4 p-3 overflow-y-scroll bg-[#E8F1FF]">
				...
				{messaageHistory.map((message) => {
					...
					if (message.sender === userIdSesion) {
						return <SenderMessage key={message.id} message={message.message} />;
					} else {
						return <RecipientMessage key={message.id} message={message.message} />;
					}
				})}
			</div>
